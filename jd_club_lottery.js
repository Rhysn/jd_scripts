...
$.brandActivityId2 = '2PTXhrEmiMEL3mD419b8Gn9bUBiJ';//超级品牌日活动ID2
...
async function superBrandDoTaskFun() {
  ...
  for (let item of $.taskList) {
    ...
    if (item['assignmentType'] === 2) {
      ...
      console.log(`${item['assignmentName']}好友encryptAssignmentId：${item['encryptAssignmentId']}`);
      console.log(`${item['assignmentName']}好友assignmentType：${item['assignmentType']}`);
    ...
    $.get({url: "https://rules.allgreat.xyz/Scripts/JD/InviteCodes/jd_club_lottery.json",headers:{
      ...
      try {
        if (err) {
          ...
          $.teamShareCodes = JSON.parse(data)
        }
      } catch (e) {
        $.logErr(e, resp)
      } finally {
