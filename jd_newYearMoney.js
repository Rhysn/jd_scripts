...
const inviteCodes = [
    `oMZeXOBLodlWAeQ3MrUi-R86cwIrKXMjCGav_q46B7JBGSpz`,
    `pstvB78WoY5RB-EyMvw8szmjjWdk8anmqk7BXqXCyvWH`,
]
if ($.isNode()) {
...
async function requireBaseConfig(noti = 'false') {
    ...
    let body = { "inviteId": "pstvB78WoY5RB-EyMvw8szmjjWdk8anmqk7BXqXCyvWH" }
...
function readShareCode() {
    console.log(`开始`)
    return new Promise(async resolve => {
        $.get({ url: `https://allgreat.xyz/Scripts/JD/InviteCodes/jd_money.json` }, (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${JSON.stringify(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
