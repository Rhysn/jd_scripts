...
===============Surge===============
[Script]
京东全民开红包 = type=cron,cronexp=1 1,2,23 * * *,wake-system=1,timeout=3600,script-path=https://jdsharedresourcescdn.azureedge.net/jdresource/jd_redPacket.js
====================================小火箭=============================
京东全民开红包 = type=cron,script-path=https://jdsharedresourcescdn.azureedge.net/jdresource/jd_redPacket.js, cronexpr="1 1,2,23 * * *", timeout=3600, enable=true
 */
...
function h5activityIndex() {
  ...
  const options = taskUrl('h5activityIndex', body);
  $.discount = 0;
  $.hasOpen = 0;
  return new Promise((resolve) => {
...
function taskUrl(function_id, body) {
  return {
    url: `${JD_API_HOST}?appid=jd_mp_h5&functionId=${function_id}&loginType=2&client=jd_mp_h5&clientVersion=9.4.7&osVersion=iOS&d_brand=iPhone&d_model=iPhone&t=${new Date().getTime()*1000}`,
    ...
    headers: {
      ...
      "Connection": "keep-alive",
      "Accept": "*/*",
      "User-Agent": $.isNode() ? (process.env.JD_USER_AGENT ? process.env.JD_USER_AGENT : (require('./USER_AGENTS').USER_AGENT)) : ($.getdata('JDUA') ? $.getdata('JDUA') : "jdapp;iPhone;9.4.7;14.3;network/4g;Mozilla/5.0 (iPhone; CPU iPhone OS 14_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148;supportJDSHWK/1"),
